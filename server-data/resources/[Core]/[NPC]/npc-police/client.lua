...
function deleteVeh(ent)
	...
	local finished = exports["npc-taskbar"]:taskBar(7500,"Impounding",false,true)	
...
AddEventHandler('impoundVeh', function()
	...
	if job == "auto_bodies" or job == 'paleto_mech' or job == 'auto_exotics' or job == 'tuner_shop' or job == "ems" or job == "police" or job == "car_shop" then
		...
		if DoesEntityExist(vehicle) and NetworkHasControlOfEntity(vehicle) then
			...
			TriggerServerEvent("npc-imp:ImpoundCar", licensePlate)
			TriggerEvent("animation:impound")
			FreezeEntityPosition(PlayerPedId(),true)
			local finished = exports["npc-taskbar"]:taskBar("3000","Completing Task")
			if finished == 100 then
				ClearPedTasks(PlayerPedId())
				FreezeEntityPosition(PlayerPedId(),false)
				deleteVeh(vehicle)
...
AddEventHandler("police:bill", function()
    local bill = exports["npc-applications"]:KeyboardInput({
...
AddEventHandler("serial:search", function()
    local serialS = exports["npc-applications"]:KeyboardInput({
...
AddEventHandler("fired:success", function(cidsent)
	...
	if tonumber(cid) == tonumber(cidsent) then
		...
		TriggerEvent("npc-jobmanager:playerBecameJob", "Unemployed", "Unemployed", false)
	end
end)