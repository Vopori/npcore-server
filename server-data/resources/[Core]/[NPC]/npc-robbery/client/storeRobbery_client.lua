...
AddEventHandler("store:dosafe", function()
	...
	if police >= 2 then
		...
		TriggerEvent("npc-dispatch:storerobbery")
...
Citizen.CreateThread(function()
	exports["npc-polyzone"]:AddBoxZone("store_robbery_lucky", vector3(913.97, -1272.84, 27.1), 1.8, 1.2, {
...
AddEventHandler('npc-polyzone:enter', function(name)
    if name == "store_robbery_lucky" then
        LuckySpot = true     
        LuckyDrawPlace()
		TriggerEvent('npc-ui:ShowUI', 'show', ("[E] %s"):format("Talk")) 
    end
end)
	...
	Citizen.CreateThread(function()
        while LuckySpot do
			...
			if IsControlJustReleased(0, 38) then
				if exports["npc-inventory"]:hasEnoughOfItem("pix1",1,false) then			
					TriggerEvent("inventory:removeItem", "pix1", 1)
					FreezeEntityPosition(PlayerPedId(),true)
					TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_HANG_OUT_STREET", 0, true)
					local finished = exports["npc-taskbar"]:taskBar(7500,"Gathering Information")
