...
document.onkeyup = function (data) {
  if (data.which == 27 ) {
    $.post('https://npc-keypad/close', JSON.stringify({}));
  } else if (data.which == 13 ) {
    $.post('https://npc-keypad/complete', JSON.stringify({ pin: $( "#PINbox" ).val() }));
  } else {
    if ( !isNaN(data.key) ) {
      var v = $( "#PINbox" ).val();
...
function submitForm(e) {
  if (e.value == "") {
     ...
     $.post('https://npc-keypad/complete', JSON.stringify({ pin:e.value }));
    data = {
      pin: e.value
    }
    /*    
