...
RegisterNetEvent('npc-vehicle.checkOwner')
AddEventHandler('npc-vehicle.checkOwner', function(status)
  checkPlayerOwnedVehicles = status
end)

RegisterNetEvent('npc-vehicle.updateVehicleDegredation')
AddEventHandler('npc-vehicle.updateVehicleDegredation', function(br,ax,rad,cl,tra,elec,fi,ft)
	local tempReturn = {}
	for k, v in pairs(trackedVehicles) do
		...
		if #tempReturn > 0 then
			TriggerServerEvent('npc-vehicle.updateVehicleDegredationServer', v[1],br,ax,rad,cl,tra,elec,fi,ft)
		end		
	end
end)

RegisterNetEvent('npc-vehicle.randomDegredation')
AddEventHandler('npc-vehicle.randomDegredation', function(upperLimit,vehicle,spinAmount)
    ...
    if checkPlayerOwnedVehicles then
        ...
        TriggerServerEvent('npc-vehicle.updateVehicleDegredationServer',plate,br,ax,rad,cl,tra,elec,fi,ft)
        TriggerServerEvent('npc-vehicle.callDegredation', plate)
	end
end)

RegisterNetEvent('npc-vehicle.updateVehicleBounce')
AddEventHandler('npc-vehicle.updateVehicleBounce', function(br,ax,rad,cl,tra,elec,fi,ft,plate)
	if br == 0 then br = nil end
	TriggerServerEvent('npc-vehicle.updateVehicleDegredationServer',plate,br,ax,rad,cl,tra,elec,fi,ft)
end)

RegisterNetEvent('npc-vehicle.getSQL')
AddEventHandler('npc-vehicle.getSQL', function(degredation)
...
AddEventHandler('veh:requestUpdate', function()
	...
	TriggerServerEvent('npc-vehicle.examine',plate,targetVehicle)
end)
...
RegisterNetEvent('npc-vehicle.getVehicleDegredation')
AddEventHandler('npc-vehicle.getVehicleDegredation', function(currentVehicle,tick)
		degHealth = getDegredationArray()
...
function trackVehicleHealth()
	...
	if #tempReturn > 0 then
		TriggerServerEvent('npc-vehicle.updateVehicleHealth', tempReturn)
	end
end

RegisterNetEvent('npc-vehicle.setVehicleHealth')
AddEventHandler('npc-vehicle.setVehicleHealth', function(eh, bh, Fuel, veh)
		...
		if IsPedInVehicle(PlayerPedId(),currentVehicle, false) then
            ...
            if playerPed == driverPed then
                if calledplate ~= GetVehicleNumberPlateText(currentVehicle) then
                    ...
                    TriggerServerEvent('npc-vehicle.callDegredation', GetVehicleNumberPlateText(currentVehicle)) 
			...
			if tick >= 15 then
				TriggerEvent('npc-vehicle.getVehicleDegredation',currentVehicle,tick)
                trackVehicleHealth()
			...
			if rTick >= 60 then
                TriggerEvent('npc-vehicle.randomDegredation',1,currentVehicle,3)
				rTick = 0
			end

		else
			if not exitveh then	
                TriggerEvent('npc-vehicle.getVehicleDegredation',lastvehicle,15)
				tick = 0
				rTick = 0
				lastvehicle = 0
				currentVehicle = 0
...
RegisterNetEvent('npc-vehicle.isPlayers')
AddEventHandler('npc-vehicle.isPlayers', function(veh,cb)
	if checkPlayerOwnedVehicles then
		cb(true)
	else
		cb(false)
...
RegisterNetEvent('npc-vehicle.getDegredation')
AddEventHandler('npc-vehicle.getDegredation', function(veh,cb)
	deghealth = getDegredationArray()
