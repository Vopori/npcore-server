        ...
        comboZone:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
            if leftZones ~= nil then
              for i = 1, #leftZones do
                TriggerEvent('npc-polyzone:exit', leftZones[i].name)
              end
            end
            if enteredZones ~= nil then
              for i = 1, #enteredZones do
                TriggerEvent('npc-polyzone:enter', enteredZones[i].name, enteredZones[i].data, enteredZones[i].center)
              end
            end
        end, 500)
    end
...
RegisterNetEvent('npc-polyzone:createCircleZone')
AddEventHandler('npc-polyzone:createCircleZone', function(name, ...)
  addCircleZone(name, ...)
end)
