    ...
    while true do
        ...
        exports.ghmattimysql:execute('DELETE FROM weed WHERE `time` - '..currTime..' < -86400', {}, function(res)
            exports.ghmattimysql:execute("SELECT `id`, `x`, `y`, `z`, `growth`, `type`, `time` FROM weed", {}, function(data)
                knownData = data
                TriggerClientEvent('npc-weed:knownCrops', -1, data, currTime)
            end)
        end)
        Wait(90000)
    end
...
AddEventHandler('playerJoining', function()
    TriggerClientEvent('npc-weed:knownCrops', source, knownData, os.time()) 
end)

RegisterNetEvent('npc-weed:killplant')
AddEventHandler('npc-weed:killplant', function(sentId) 
    exports.ghmattimysql:execute("DELETE FROM weed WHERE id = @id", {['@id'] = sentId}, function()
            TriggerClientEvent('npc-weed:updatePlant', -1, 0, sentId)
    end) 
end)

RegisterNetEvent('npc-weed:newplant')
AddEventHandler('npc-weed:newplant', function(sentCoords, sentType)
    local source = source
    ...
    exports.ghmattimysql:execute("INSERT INTO weed (x, y, z, type, time) VALUES (@x, @y, @z, @type, @time)", { 
        ...
        if affectedRows.affectedRows > 0 then
            if affectedRows.insertId then
                ...
                TriggerClientEvent('npc-weed:updatePlant', -1, 3, affectedRows.insertId, 0, 0, 3, sentNewInfo, os.time())
...
RegisterNetEvent('npc-weed:updateStatus')
AddEventHandler('npc-weed:updateStatus', function(updateType, sentId, sentData, sentItem)
    if updateType == 'update' and sentData then
        ...
        exports.ghmattimysql:execute("UPDATE weed SET growth = @growth, time = @time WHERE id = @id", { 
            ...
            ['@time'] = newTime-- 12 hours
                ...
                TriggerClientEvent('npc-weed:updatePlant', -1, 1, sentId, sentData, newTime, 1, {}, tonumber(os.time()))
        end)
    end
end)