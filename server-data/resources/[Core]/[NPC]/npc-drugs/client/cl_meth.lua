...
AddEventHandler("meth:start", function()
	...
	if exports["npc-inventory"]:hasEnoughOfItem("acetone",5,false) and exports["npc-inventory"]:hasEnoughOfItem("sudafed",2,false) and exports["npc-inventory"]:hasEnoughOfItem("antifreeze",3,false) and ongoing == false then
		TriggerEvent("animation:meth")
		FreezeEntityPosition(PlayerPedId(),true)
		SetEntityHeading(PlayerPedId(), 178.82402038574)
		local finished = exports["npc-taskbar"]:taskBar(10000,"Putting Materials Into The Cooker!")
		if (finished == 100) then
			...
			if math.random(1,30) == 18  then
				...
				TriggerEvent("npc-dispatch:methexplosion")
				DaBabyBoom()
			else
				TriggerEvent("inventory:removeItem","acetone", 5)
				TriggerEvent("inventory:removeItem","sudafed", 2)
...
AddEventHandler("meth:sell", function()
	if exports["npc-inventory"]:hasEnoughOfItem("meth",1,false) then
		local finished = exports["npc-taskbar"]:taskBar(3000,"Handing Over Something")
		if (finished == 100) then
...
AddEventHandler("stopcooking", function()
	if cooking == true then
		TriggerEvent("DoLongHudText", "Failed, You Left The Meth Lab Too Early!")
		cooking = false
		ongoing = false
	end
end)
