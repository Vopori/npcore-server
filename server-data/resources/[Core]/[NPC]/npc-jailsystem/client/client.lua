...
function RoleplayStats()
    ...
    if exports["urp-inventory"]:hasEnoughOfItem("shitlockpick",1,false) then
        totalroleplay = totalroleplay + 10
    end

    if exports["urp-inventory"]:hasEnoughOfItem("jailfood",1,false) then
        totalroleplay = totalroleplay + 10
    end

    if exports["urp-inventory"]:hasEnoughOfItem("methbag",1,false) then
        totalroleplay = totalroleplay + 15
    end

    if exports["urp-inventory"]:hasEnoughOfItem("assphone",1,false) then
        totalroleplay = totalroleplay + 15
    ...
    if exports["urp-inventory"]:hasEnoughOfItem("slushy",1,false) then
        totalroleplay = totalroleplay + 10
    end
...
function InmateHasAll()
    if exports["urp-inventory"]:hasEnoughOfItem("slushy",1,false)
        and
        exports["urp-inventory"]:hasEnoughOfItem("-1810795771",1,false)
        and
        exports["urp-inventory"]:hasEnoughOfItem("methbag",1,false)
        and
        exports["urp-inventory"]:hasEnoughOfItem("assphone",1,false)
        and
        exports["urp-inventory"]:hasEnoughOfItem("slushy",1,false)
    then
        return true
    else
        return false
        ...
        while imjailed do
            ...
            if (#(GetEntityCoords(playerPed, 0) - vector3(1642.08, 2522.16, 45.57)) < 1.0) then
                ...
                if IsControlJustPressed(1, Controlkey["generalUse"][1]) then
                    TriggerEvent("urp-ac:triggeredItemSpawn", "997", "Shop");                    
                    Citizen.Wait(5000)
                end
            end
...
RegisterCommand('jail', function(source, args)
    ...
    if exports['isPed']:isPed('job') == 'Police' then
        if args[1] then
            if args[2] then
                TriggerServerEvent('urp-jailhehe', args[1], args[2], name, cid)
                TriggerServerEvent('urp-business:givepass', 'Police', math.random(800, 1000))
            else
