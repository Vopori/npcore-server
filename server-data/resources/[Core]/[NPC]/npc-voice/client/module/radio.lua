...
RegisterNetEvent('npc-voice:syncRadioData', syncRadioData)
...
RegisterNetEvent('npc-voice:addPlayerToRadio', addPlayerToRadio)
...
RegisterNetEvent('npc-voice:removePlayerFromRadio', removePlayerFromRadio)
...
function setRadioChannel(channel)
	...
	TriggerServerEvent('npc-voice:setPlayerRadio', channel)
...
RegisterCommand('+radiotalk', function()
	...
	if not radioPressed and radioEnabled then
		if radioChannel > 0 then
			...
			playerTargets(radioData, NetworkIsPlayerTalking(PlayerId()) and callData or {})
			TriggerServerEvent('npc-voice:setTalkingOnRadio', true)
			radioPressed = true
			playMicClicks(true)
			...
			Citizen.CreateThread(function()
				TriggerEvent("npc-voice:radioActive", true)
...
RegisterCommand('-radiotalk', function()
	if radioChannel > 0 or radioEnabled then
		...
		playerTargets(NetworkIsPlayerTalking(PlayerId()) and callData or {})
		TriggerEvent("npc-voice:radioActive", false)
		playMicClicks(false)
		if GetConvarInt('voice_enableRadioAnim', 0) == 1 then
			StopAnimTask(PlayerPedId(), "random@arrests", "generic_radio_enter", -4.0)
		end
		TriggerServerEvent('npc-voice:setTalkingOnRadio', false)
...
RegisterNetEvent('npc-voice:clSetPlayerRadio', syncRadio)
