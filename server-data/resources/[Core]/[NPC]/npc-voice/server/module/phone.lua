...
function removePlayerFromCall(source, callChannel)
    ...
    for player, _ in pairs(callData[callChannel]) do
        TriggerClientEvent('npc-voice:removePlayerFromCall', player, source)
    end
...
function addPlayerToCall(source, callChannel)
    ...
    for player, _ in pairs(callData[callChannel]) do
        ...
        if player ~= source then
            TriggerClientEvent('npc-voice:addPlayerToCall', player, source)
        end
    end
    callData[callChannel][source] = false
    voiceData[source] = voiceData[source] or defaultTable(source)
    voiceData[source].call = callChannel
    TriggerClientEvent('npc-voice:syncCallData', source, callData[callChannel])
end
...
function setPlayerCall(source, callChannel)
    ...
    if GetInvokingResource() then
        ...
        -- changed
        TriggerClientEvent('npc-voice:clSetPlayerCall', source, callChannel)
    end
    voiceData[source] = voiceData[source] or defaultTable(source)
...
RegisterNetEvent('npc-voice:setPlayerCall', function(callChannel)
    ...
    if callTbl then
        ...
        for player, _ in pairs(callTbl) do
            if player ~= source then
                ...
                TriggerClientEvent('npc-voice:setTalkingOnCall', player, source, talking)
            end
        end
    else
...
RegisterNetEvent('npc-voice:setTalkingOnCall', setTalkingOnCall)