local DISCORD_WEBHOOK5 = "https://discord.com/api/webhooks/856840221747052565/D-JvTpi2B1TV3pmTLrrevwNkrg2TzoBO3pp2SzOd0FKVzuohVDqmmvEH8OZEbpLVLEWg"
...
function discordLog(message, color, channel)
  if Config.AllLogs then
	PerformHttpRequest(Config.webhooks["all"], function(err, text, headers) end, 'POST', json.encode({username = Config.username, embeds = {{["color"] = color, ["author"] = {["name"] = Config.communtiyName,["icon_url"] = Config.communtiyLogo}, ["description"] = "".. message .."",["footer"] = {["text"] = "© NPCore Framework - "..os.date("%x %X %p"),["icon_url"] = "https://cdn.discordapp.com/attachments/851633925624168479/851634078243094558/NPCore_Base_GH.png",},}}, avatar_url = Config.avatar}), { ['Content-Type'] = 'application/json' })
  end
	PerformHttpRequest(Config.webhooks[channel], function(err, text, headers) end, 'POST', json.encode({username = Config.username, embeds = {{["color"] = color, ["author"] = {["name"] = Config.communtiyName,["icon_url"] = Config.communtiyLogo}, ["description"] = "".. message .."",["footer"] = {["text"] = "© NPCore Framework - "..os.date("%x %X %p"),["icon_url"] = "https://cdn.discordapp.com/attachments/851633925624168479/851634078243094558/NPCore_Base_GH.png",},}}, avatar_url = Config.avatar}), { ['Content-Type'] = 'application/json' })
end
...
AddEventHandler('weaponDamageEvent', function(sender, data)
    if data.weaponDamage > 60 then
        DropPlayer(source, "You have been automatically kicked for using Damage Multipliers! If this was a false-positive, please let the NPCore Staff Team know so they can fix this issue!")
