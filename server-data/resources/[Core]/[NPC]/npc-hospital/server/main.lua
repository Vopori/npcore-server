...
RegisterServerEvent('npc-hospital:server:RequestBed')
AddEventHandler('npc-hospital:server:RequestBed', function()
    for k, v in pairs(beds) do
        if not v.taken then
            v.taken = true
            bedsTaken[source] = k
            TriggerClientEvent('npc-hospital:client:SendToBed', source, k, v)
            return
        end
    end
...
RegisterServerEvent('npc-hospital:server:RPRequestBed')
AddEventHandler('npc-hospital:server:RPRequestBed', function(plyCoords)
    ...
    for k, v in pairs(beds) do
        ...
        if distance < 3.0 then
            if not v.taken then
                v.taken = true
                foundbed = true
                TriggerClientEvent('npc-hospital:client:RPSendToBed', source, k, v)
                return
            else
...
RegisterServerEvent('npc-hospital:server:EnteredBed')
AddEventHandler('npc-hospital:server:EnteredBed', function()
    local src = source
    ...
    TriggerClientEvent('npc-hospital:client:FinishServices', src)
end)

RegisterServerEvent('npc-hospital:server:LeaveBed')
AddEventHandler('npc-hospital:server:LeaveBed', function(id)
    beds[id].taken = false
end)