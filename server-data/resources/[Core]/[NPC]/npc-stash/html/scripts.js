...
function pinFailure() {
  $.post('https://npc-stash/failure', JSON.stringify({pinResult: false}));
  gameOver = true;
  $('#modal').fadeIn();
  ++failedAttempts;
  if (failedAttempts >= 3)
...
function pinSuccess(pElevated) {
  $.post('https://npc-stash/complete', JSON.stringify({pinResult: true, owner: pElevated}));
...
document.onkeyup = function (data) {
  if (data.which == 27 ) {
    $.post('https://npc-stash/close', JSON.stringify({pinResult: false}));
  }
};
