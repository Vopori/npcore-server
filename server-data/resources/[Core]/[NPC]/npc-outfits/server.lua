...
RegisterServerEvent("npc-outfits:insert_character_current")
AddEventHandler("npc-outfits:insert_character_current",function(data)
...
RegisterServerEvent("npc-outfits:insert_character_face")
AddEventHandler("npc-outfits:insert_character_face",function(data)
    if not data then return end
    local src = source
...
RegisterServerEvent("npc-outfits:get_character_face")
AddEventHandler("npc-outfits:get_character_face",function(pSrc)
    ...
    exports.ghmattimysql:execute("SELECT cc.model, cf.hairColor, cf.headBlend, cf.headOverlay, cf.headStructure FROM character_face cf INNER JOIN character_current cc on cc.cid = cf.cid WHERE cf.cid = @cid", {['cid'] = characterId}, function(result)
        if (result ~= nil and result[1] ~= nil) then
            ...
            if model == 1885233650 or model == -1667301416 then
                TriggerClientEvent("npc-outfits:setpedfeatures", src, temp_data)
            end
        else
            TriggerClientEvent("npc-outfits:setpedfeatures", src, false)
        end
	end)
end)

RegisterServerEvent("npc-outfits:get_character_current")
AddEventHandler("npc-outfits:get_character_current",function(pSrc)
    local src = (not pSrc and source or pSrc)
    local user = exports["npc-core"]:getModule("Player"):GetUser(src)
    ...
    exports.ghmattimysql:execute("SELECT * FROM character_current WHERE cid = @cid", {['cid'] = cid}, function(result)
        ...
        TriggerClientEvent("npc-outfits:setclothes", src, temp_data, 0)
	end)
end)

RegisterServerEvent("npc-outfits:retrieve_tats")
AddEventHandler("npc-outfits:retrieve_tats", function(pSrc)
    local src = (not pSrc and source or pSrc)
	local user = exports["npc-core"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
...
RegisterServerEvent("npc-outfits:set_tats")
AddEventHandler("npc-outfits:set_tats", function(tattoosList)
	local src = source
	local user = exports["npc-core"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
...
RegisterServerEvent("npc-outfits:get_outfit")
AddEventHandler("npc-outfits:get_outfit",function(slot)
    ...
    exports.ghmattimysql:execute("SELECT * FROM character_outfits WHERE cid = @cid and slot = @slot", {
        ...
        if result and result[1] then
            ...
            TriggerClientEvent("npc-outfits:setclothes", src, data , 0)
...
RegisterServerEvent("npc-outfits:set_outfit")
AddEventHandler("npc-outfits:set_outfit",function(slot, name, data)
    if not slot then return end
    local src = source
...
RegisterServerEvent("npc-outfits:remove_outfit")
AddEventHandler("npc-outfits:remove_outfit",function(slot)
...
RegisterServerEvent("npc-outfits:list_outfits")
AddEventHandler("npc-outfits:list_outfits",function()
    local src = source
    local user = exports["npc-core"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local slot = slot
    ...
    exports.ghmattimysql:execute("SELECT slot, name FROM character_outfits WHERE cid = @cid", {['cid'] = cid}, function(skincheck)  
        for i = 1, #skincheck do
            TriggerClientEvent('npc-context:sendMenu', src, {
                {
                    id = skincheck[i].slot,
                    header = "Name: " ..skincheck[i].name,
                    txt = "",
                    params = {
                        event = "npc-outfits:list:outfits",
                        args = {
    ...
    exports.ghmattimysql:execute("SELECT count(rank) whitelist FROM jobs_whitelist WHERE cid = @cid LIMIT 1", {
        ...
        exports.ghmattimysql:scalar("SELECT count(model) FROM character_current WHERE cid = @cid LIMIT 1", {
            ...
            if result == 0 then
                ...
                TriggerEvent("npc-outfits:get_character_current", src)
            end
            TriggerClientEvent("npc-outfits:inService",src,isService)
    	end)
    end)
end)
