...
local inSpawn = false
AddEventHandler("npc-outfits:inSpawn", function(pInSpawn)
    inSpawn = pInSpawn
end)

local statusS = false
RegisterNetEvent("npc-outfits:Spawning")
AddEventHandler("npc-outfits:Spawning", function(pStatus)
    statusS = pStatus
end)
...
function Save(save, close)
    ...
    if save then
        ...
        if not startingMenu or passedClothing then
            TriggerServerEvent("npc-outfits:insert_character_current", data)
            TriggerServerEvent("npc-outfits:insert_character_face", data)
            TriggerServerEvent("npc-outfits:set_tats", currentTats)
...
RegisterNetEvent("npc-outfits:inService")
AddEventHandler("npc-outfits:inService", function(service)
    isService = service
end)

RegisterNetEvent("npc-outfits:hasEnough")
AddEventHandler("npc-outfits:hasEnough", function(menu)
    if menu == "tattoo_shop" then
        TriggerServerEvent("npc-outfits:retrieve_tats")
...
RegisterNetEvent("npc-outfits:setclothes")
AddEventHandler("npc-outfits:setclothes", function(data,alreadyExist)
    player = PlayerPedId()
    ...
    Citizen.Wait(500)
	TriggerEvent("facewear:update")
    TriggerServerEvent("npc-outfits:get_character_face")
    TriggerServerEvent("npc-outfits:retrieve_tats")
	TriggerServerEvent("police:SetMeta")
    TriggerEvent("e-blips:updateAfterPedChange",exports["isPed"]:isPed("myjob"))
end)

RegisterNetEvent("npc-outfits:AdminSetModel")
AddEventHandler("npc-outfits:AdminSetModel", function(model)
...
RegisterNetEvent("npc-outfits:settattoos")
AddEventHandler("npc-outfits:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("npc-outfits:setpedfeatures")
AddEventHandler("npc-outfits:setpedfeatures", function(data)
    player = PlayerPedId()
    if data == false then
...
RegisterNetEvent('npc-outfits:ListOutfits')
AddEventHandler('npc-outfits:ListOutfits', function(skincheck)
  local menuData = {}
...
RegisterNetEvent('npc-outfits:openClothing')
AddEventHandler('npc-outfits:openClothing', function(pDontShowBarber, pShouldCost)
...
AddEventHandler("npc-polyzone:enter", function(zone, data)
    ...
    if currentZone then
        ...
        TriggerEvent("npc-outfits:enable", true)
...
AddEventHandler("npc-polyzone:exit", function(zone)
    ...
    if currentZone then
        listening = false
        TriggerEvent('npc-ui:HideUI')
        TriggerEvent("npc-outfits:enable", false)
    end
end)

RegisterNetEvent('npc-outfits:outfits')
AddEventHandler('npc-outfits:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("npc-outfits:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("npc-outfits:remove_outfit",pId)
    elseif pAction == 3 then 
        TriggerEvent('item:deleteClothesDna')
        TriggerEvent('InteractSound_CL:PlayOnOne','Clothes1', 0.6)
        TriggerServerEvent("npc-outfits:get_outfit", pId)
    else
...
RegisterNetEvent('npc-outfits:enable')
AddEventHandler('npc-outfits:enable', function(status)
    actionDress = status
end)
...
RegisterCommand("outfitadd", function(source, args, rawCommand)
    if actionDress == true then
        if args[1] and args[2] then
            TriggerEvent('npc-outfits:outfits', 1, args[1], args[2])
        else
...
RegisterCommand("outfitdel", function(source, args, rawCommand)
    if actionDress == true then
        if args[1] then
            TriggerEvent('npc-outfits:outfits', 2, args[1])
        else
...
RegisterCommand("outfits", function(source, args, rawCommand)
    if actionDress == true then
        TriggerEvent('npc-outfits:outfits', 4)
    else
        TriggerEvent('DoLongHudText', "You are not near a wardrobe", 2)
    end
end, false)

RegisterNetEvent("npc-outfits:list:outfits")
AddEventHandler("npc-outfits:list:outfits", function(data)
    TriggerServerEvent("npc-outfits:get_outfit", data.slot)
end)
