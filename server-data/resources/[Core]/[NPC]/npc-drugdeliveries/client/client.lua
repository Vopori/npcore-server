...
Citizen.CreateThread(function()
	exports["npc-polyzone"]:AddBoxZone("oxy_run_start", vector3(510.71, -1951.51, 24.99), 1.8, 1, {
		name="oxy_run_start",
		heading=35,
		--debugPoly=true,
		minZ=23.99,
...
AddEventHandler('npc-polyzone:enter', function(name)
    if name == "oxy_run_start" then
        ...
        LuckyDrawPlace()
		if not OxyRun then
			TriggerEvent('npc-ui:ShowUI', 'show', ("[E] %s"):format("Start Run ($750)")) 
		else
			TriggerEvent('npc-ui:ShowUI', 'show', ("%s"):format("Finish Your Run")) 
		end
    end
end)
...
function DoDropOff(requestMoney)
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
			...
			if OxyRun then
		        ...
		        elseif exports["npc-inventory"]:hasEnoughOfItem("rollcash",10,false) then     
				...
				elseif exports["npc-inventory"]:hasEnoughOfItem("band",10,false) then     
