...
RegisterCommand('boat', function(source, args)
	if exports["isPed"]:isPed("myjob") == "police" then
      TriggerEvent('npc:spawnVehicle', 'predator')
	else
...
RegisterNetEvent('npc-spawnveh:showPD')
AddEventHandler('npc-spawnveh:showPD', function()
    showPD = not showPD
...
RegisterNetEvent('npc-spawnveh:EMS')
AddEventHandler('npc-spawnveh:EMS', function()
	...
	for k,v in pairs(Config.PoliceGarage) do 
		if IsPedInAnyVehicle(ped, true) then
			...
			if GetDistanceBetweenCoords(GetEntityCoords(ped), v[1], v[2], v[3], true) <= Config.Distance then
				...
				local finished = exports["npc-taskbar"]:taskBar(5000, "Completing Task")
				if finished == 100 then
					TriggerEvent('DoLongHudText', 'Your vehicle has been repaired', 1)
					SetVehicleFixed(veh)
					SetVehicleDirtLevel(veh, 0.0)
					exports["npc-oGasStations"]:SetFuel(veh, 100)
...
Citizen.CreateThread(function()
    showPD = false
    TriggerEvent('npc-spawnveh:showPD')
end)

Citizen.CreateThread(function()
    showHospitals = false
    TriggerEvent('npc-spawnveh:EMS')
end)