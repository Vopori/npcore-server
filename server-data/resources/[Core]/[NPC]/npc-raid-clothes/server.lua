...
RegisterServerEvent("npc-raid-clothes:insert_character_current")
...
RegisterServerEvent("npc-raid-clothes:insert_character_face")
AddEventHandler("npc-raid-clothes:insert_character_face",function(data)
    if not data then return end
    local src = source
    ...
    exports.ghmattimysql:execute("SELECT cc.model, cf.hairColor, cf.headBlend, cf.headOverlay, cf.headStructure FROM character_face cf INNER JOIN character_current cc on cc.cid = cf.cid WHERE cf.cid = @cid", {['cid'] = characterId}, function(result)
        if (result ~= nil and result[1] ~= nil) then
            ...
            if model == 1885233650 or model == -1667301416 then
                TriggerClientEvent("npc-raid-clothes:setpedfeatures", src, temp_data)
            end
        else
            TriggerClientEvent("npc-raid-clothes:setpedfeatures", src, false)
        end
	end)
end)
...
AddEventHandler("npc-raid-clothes:get_character_current",function(pSrc)
    ...
    exports.ghmattimysql:execute("SELECT * FROM character_current WHERE cid = @cid", {['cid'] = cid}, function(result)
        ...
        TriggerClientEvent("npc-raid-clothes:setclothes", src, temp_data, 0)
	end)
end)
...
AddEventHandler("npc-raid-clothes:retrieve_tats", function(pSrc)
	...
	exports.ghmattimysql:execute("SELECT * FROM playersTattoos WHERE identifier = @identifier", {['identifier'] = char.id}, function(result)
        if(#result == 1) then
			TriggerClientEvent("npc-raid-clothes:settattoos", src, json.decode(result[1].tattoos))
		else
			local tattooValue = "{}"
			...
			TriggerClientEvent("npc-raid-clothes:settattoos", src, {})
		end
	end)
end)
...
RegisterServerEvent("npc-raid-clothes:get_outfit")
AddEventHandler("npc-raid-clothes:get_outfit",function(slot)
    ...
    exports.ghmattimysql:execute("SELECT * FROM character_outfits WHERE cid = @cid and slot = @slot", {
        ...
        if result and result[1] then
            ...
            TriggerClientEvent("npc-raid-clothes:setclothes", src, data , 0)
...
RegisterServerEvent("npc-raid-clothes:set_outfit")
AddEventHandler("npc-raid-clothes:set_outfit",function(slot, name, data)
    if not slot then return end
    local src = source
...
RegisterServerEvent("npc-raid-clothes:remove_outfit")
            ...
            TriggerClientEvent('npc-context:sendMenu', src, {
                {
                    ...
                    txt = "",
                    params = {
                        event = "npc-raid-clothes:list:outfits",
                        args = {
    ...
    exports.ghmattimysql:execute("SELECT count(rank) whitelist FROM jobs_whitelist WHERE cid = @cid LIMIT 1", {
        ...
        exports.ghmattimysql:scalar("SELECT count(model) FROM character_current WHERE cid = @cid LIMIT 1", {
            ...
            if result == 0 then
                ...
                TriggerEvent("npc-raid-clothes:get_character_current", src)
            end
            TriggerClientEvent("npc-raid-clothes:inService",src,isService)
    	end)
    end)
end)
