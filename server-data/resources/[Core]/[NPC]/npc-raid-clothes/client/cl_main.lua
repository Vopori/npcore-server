...
local inSpawn = false
AddEventHandler("npc-raid-clothes:inSpawn", function(pInSpawn)
    inSpawn = pInSpawn
end)

local statusS = false
RegisterNetEvent("npc-raid-clothes:Spawning")
AddEventHandler("npc-raid-clothes:Spawning", function(pStatus)
    statusS = pStatus
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if statusS == true then
            TriggerServerEvent("npc-login:inspawn", true)
        end
    end
end)
...
function Save(save, close)
    ...
    if save then
        ...
        if not startingMenu or passedClothing then
            TriggerServerEvent("npc-raid-clothes:insert_character_current", data)
            TriggerServerEvent("npc-raid-clothes:insert_character_face", data)
            TriggerServerEvent("npc-raid-clothes:set_tats", currentTats)
            TriggerEvent("npc-login:finishedClothing","Finished")
        ...
        TriggerEvent("npc-login:finishedClothing","Old")
        LoadPed(oldPed)
    end
...
RegisterNetEvent("npc-raid-clothes:inService")
AddEventHandler("npc-raid-clothes:inService", function(service)
    isService = service
end)

RegisterNetEvent("npc-raid-clothes:hasEnough")
AddEventHandler("npc-raid-clothes:hasEnough", function(menu)
    if menu == "tattoo_shop" then
        TriggerServerEvent("npc-raid-clothes:retrieve_tats")
...
RegisterNetEvent("npc-raid-clothes:setclothes")
AddEventHandler("npc-raid-clothes:setclothes", function(data,alreadyExist)
    player = PlayerPedId()
    ...
    Citizen.Wait(500)
	TriggerEvent("facewear:update")
    TriggerServerEvent("npc-raid-clothes:get_character_face")
    TriggerServerEvent("npc-raid-clothes:retrieve_tats")
	TriggerServerEvent("police:SetMeta")
    TriggerEvent("e-blips:updateAfterPedChange",exports["isPed"]:isPed("myjob"))
end)

RegisterNetEvent("npc-raid-clothes:AdminSetModel")
AddEventHandler("npc-raid-clothes:AdminSetModel", function(model)
...
RegisterNetEvent("npc-raid-clothes:defaultReset")
AddEventHandler("npc-raid-clothes:defaultReset", function()
...
RegisterNetEvent("npc-raid-clothes:settattoos")
AddEventHandler("npc-raid-clothes:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("npc-raid-clothes:setpedfeatures")
AddEventHandler("npc-raid-clothes:setpedfeatures", function(data)
    player = PlayerPedId()
    if data == false then
...
RegisterNetEvent('npc-raid-clothes:ListOutfits')
AddEventHandler('npc-raid-clothes:ListOutfits', function(skincheck)
  local menuData = {}
    ...
    menuData[#menuData + 1] = {
      ...
      description = '',
      action = 'np-ui:npc-raid-clothes:changeOutfit',
      key = i,
    }
  end
  TriggerEvent('npc-ui:ShowUI', 'show', menuData)
end)
...
RegisterNetEvent('npc-raid-clothes:openClothing')
AddEventHandler('npc-raid-clothes:openClothing', function(pDontShowBarber, pShouldCost)
...
AddEventHandler("npc-polyzone:enter", function(zone, data)
    local currentZone = MenuData[zone]
    if currentZone then
        TriggerEvent('npc-ui:ShowUI', 'show', ("[M] %s"):format(currentZone.text))
        TriggerEvent("npc-raid-clothes:enable", true)
        listenForKeypress(zone, currentZone, ((data and data.isFree) and true or false))
    end
end)

AddEventHandler("npc-polyzone:exit", function(zone)
    local currentZone = MenuData[zone]
    if currentZone then
        listening = false
        TriggerEvent('npc-ui:HideUI')
        TriggerEvent("npc-raid-clothes:enable", false)
    end
end)

RegisterNetEvent('npc-raid-clothes:outfits')
AddEventHandler('npc-raid-clothes:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("npc-raid-clothes:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("npc-raid-clothes:remove_outfit",pId)
    elseif pAction == 3 then 
        TriggerEvent('item:deleteClothesDna')
        TriggerEvent('InteractSound_CL:PlayOnOne','Clothes1', 0.6)
        TriggerServerEvent("npc-raid-clothes:get_outfit", pId)
    else
...
RegisterNetEvent('npc-raid-clothes:enable')
AddEventHandler('npc-raid-clothes:enable', function(status)
    actionDress = status
end)
...
RegisterCommand("outfitadd", function(source, args, rawCommand)
    if actionDress == true then
        if args[1] and args[2] then
            TriggerEvent('npc-raid-clothes:outfits', 1, args[1], args[2])
        else
...
RegisterCommand("outfitdel", function(source, args, rawCommand)
    if actionDress == true then
        if args[1] then
            TriggerEvent('npc-raid-clothes:outfits', 2, args[1])
        else
...
RegisterCommand("outfits", function(source, args, rawCommand)
    if actionDress == true then
        TriggerEvent('npc-raid-clothes:outfits', 4)
    else
        TriggerEvent('DoLongHudText', "You are not near a wardrobe", 2)
    end
end, false)  
...
RegisterNetEvent("npc-raid-clothes:list:outfits")
AddEventHandler("npc-raid-clothes:list:outfits", function(data)
    TriggerServerEvent("npc-raid-clothes:get_outfit", data.slot)
end)
