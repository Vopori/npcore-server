...
RegisterNetEvent('npc-casino:unlock')
AddEventHandler('npc-casino:unlock', function()
  CasinoOpen = true
end)

RegisterNetEvent('npc-casino:lock')
...
RegisterCommand('casino', function()
  if exports['isPed']:isPed('job') == 'DiamondCasino' then
    if CasinoOpen then
      TriggerServerEvent('npc-casino:lock')
      TriggerEvent('DoLongHudText', 'The Casino Is Now Closed!')
    else
      TriggerServerEvent('npc-unlock:casino')
...
AddEventHandler('onClientGameTypeStart', function()
  TriggerServerEvent('npc-casino:CheckIfOpen')
end)
...
AddEventHandler("npc-casino:elevatorEnterCasino", function()
  enterCasino(true, true)
end)
AddEventHandler("npc-casino:elevatorExitCasino", function()
    ...
    if CasinoOpen then
      if #(GetEntityCoords(PlayerPedId()) - vector3(1089.678, 205.7735, -48.99973)) < 1 then
        ...
        if IsControlJustPressed(0, 38) then
          if exports['npc-inventory']:hasEnoughOfItem('casinokey', 1) then
            inCasino = false
...
function enterCasino(pIsInCasino, pFromElevator, pCoords, pHeading)
  ...
  TriggerEvent("npc-casino:casinoEnteredEvent")
  TriggerServerEvent('npc-infinity:scopes:casino', true)
end
...
function handlePedCoordsBaby(pPedCoords)
  ...
  -- RPC.execute("npc-casino:handoffPedData", myPeds)
  Citizen.CreateThread(function()
    while inCasino do
...
-- end, false)
-- Citizen.CreateThread(function()
--   exports["npc-keybinds"]:registerKeyMapping("", "Casino", "Add Coords", "+addCasinoCoords", "-addCasinoCoords")
-- end)

-- Citizen.CreateThread(function()
--   for _, v in pairs(exports["npc-casino"]:getPedCoordsC()) do
