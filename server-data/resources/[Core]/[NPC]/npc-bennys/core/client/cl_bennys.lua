...
Citizen.CreateThread(function()
    exports["npc-polyzone"]:AddBoxZone("Civ_repair", vector3(-222.73, -1329.78, 30.89), 9.0, 5, {
        name="Civ_repair",
        heading=90,
        --debugPoly=true,
        minZ=29.89,
...
AddEventHandler('npc-polyzone:enter', function(name)
    ...
    if name == "Civ_repair" then
        if vehicle ~= 0 then
            NearCiv = true
            CivRepairFunction()
            TriggerEvent('npc-ui:ShowUI', 'show', ("[E] %s"):format("Repair ($450)"))
        end
    end
end)
        ...
        while NearCiv do
            ...
            if IsControlJustReleased(0, 38) then
                if car ~= 0 then
                    ...
                    local finished = exports["npc-taskbar"]:taskBar(24000,"Repairing")
                    if finished == 100 then
                        ...
                        TriggerServerEvent("npc-bennys:repairciv")
                    else
                        FreezeEntityPosition(car, false)
                    end
                end
