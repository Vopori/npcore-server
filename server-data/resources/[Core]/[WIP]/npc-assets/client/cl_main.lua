...
-- Discord Rich Presence (Change "RichAppId" to your desired application.)
local onlinePlayers = 0
RegisterNetEvent('rich:TakePlayers')
AddEventHandler('rich:TakePlayers', function(count)
    onlinePlayers = count
    if SetDiscordRichPresenceAction then
        SetDiscordRichPresenceAction(0, 'https://discord.gg/ahbEjHTfRm', 'https://discord.gg/ahbEjHTfRm')
    end
end)
...
Citizen.CreateThread(function()
    ...
    SetDiscordRichPresenceAsset(GetConvar("RichAssetId", "npc"))
    while true do
        Citizen.Wait(2000)
