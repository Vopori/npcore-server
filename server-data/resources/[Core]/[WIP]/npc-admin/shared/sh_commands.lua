...
function cmd.RunCommand(caller, args)
    ...
    TriggerEvent("npc-admin:update:vehicle", caller.source, args.reason)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Set State") then cmd.vars.toggle = not cmd.vars.toggle NPC.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle, reason = cmd.vars.reason}) end
end
...
function cmd.DrawCommand()
    ...
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Search Player") then cmd.vars.toggle = not cmd.vars.toggle NPC.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle,target = cmd.vars.target}) end
end
