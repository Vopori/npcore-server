...
RegisterNetEvent('npc-weed:knownCrops')
AddEventHandler('npc-weed:knownCrops', function(sentInfo, currentTime)

    local hellYeah = {}

    TriggerEvent('npc-weed:DeleteKnownCrops')
    knownCrops = hellYeah
...
AddEventHandler('npc-weed:DeleteKnownCrops', function()
...
RegisterNetEvent('npc-weed:plantSeed')
AddEventHandler('npc-weed:plantSeed', function(sentType)
    if not isBusy and inhouse then
        ...
        if veh == 0 then
            ...
            if success then
                if exports["npc-inventory"]:hasEnoughOfItem(sentType,1,false) and exports["npc-inventory"]:hasEnoughOfItem("plantpot", 1) then
                TriggerEvent("animation:farm")
                local finished = exports["npc-taskbar"]:taskBar(10000,"Planting Seed",true,false,playerVeh)
...
RegisterNetEvent('npc-weed:destroyPlant')
AddEventHandler('npc-weed:destroyPlant', function(sentType)
    if not isBusy then
        ...
        if knownInfo['id'] > 0 and knownInfo['dist'] < 4 then
            if knownCrops[knownInfo['id']]['object'] and DoesEntityExist(knownCrops[knownInfo['id']]['object']) then
                TriggerServerEvent("npc-weed:killplant", knownCrops[knownInfo['id']]['knownId'])
            end
        end
...
function NewPlant(sentType)
	TriggerServerEvent("npc-weed:newplant", GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 0.4, 0.0), sentType)
end
...
RegisterNetEvent('npc-weed:updatePlant')
AddEventHandler('npc-weed:updatePlant', function(updateType, sentId, sentGrowth, sentTime, sentStatus, sentNewInfo, currentTime)
                ...
                if cropDist <= 2 then
                    if IsControlJustReleased(0, 38) then
                        if closestInfo["growth"] >= 100 then
                            ...
                            local finished = exports["npc-taskbar"]:taskBar(1000,"Picking",false,false)
                            if (finished == 100) then
                                ClearPedTasks(GetPlayerPed(-1))
                                if currCrop and currCrop["knownId"] then
                                    TriggerEvent("npc-weed:destroyPlant")
                                    TriggerEvent("npc-weed:DeleteKnownCrops")
                                    TriggerServerEvent('npc-weed:killplant', currCrop["knownId"])
                                    DeleteObject(ObjectFound)
                                    DeleteEntity(ObjectFound)
                        ...
                        elseif num == 2 then
                            if exports["npc-inventory"]:hasEnoughOfItem("water",1,false) then
                                ...
                                TriggerServerEvent("npc-weed:updateStatus", 'update', currCrop["knownId"], new, 'water')
