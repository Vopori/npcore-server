...
CanHunt = false
RegisterNetEvent("npc-hunting:allowed")
AddEventHandler("npc-hunting:allowed", function(allowed)
    CanHunt = allowed
end)
...
function StartHuntingSession()
	if allowedHunting == true then
		...
		if OnGoingHuntSession then
			OnGoingHuntSession = false
			if exports["npc-inventory"]:hasEnoughOfItem("100416529",1,false) then
				...
				TriggerServerEvent('npc:huntingreturnree')
				StartDelay()
			end				
		else
...
AddEventHandler('blazer', function()
	...
	TriggerServerEvent("npc-hunting:giveloadout")
end)