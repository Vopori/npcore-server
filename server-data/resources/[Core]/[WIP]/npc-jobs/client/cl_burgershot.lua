...
AddEventHandler("npc-burgershot:startprocess3", function()
    ...
    if rank > 0 then    
        if exports["npc-inventory"]:hasEnoughOfItem("rawpatty", 1) then 
            ...
            local finished = exports['npc-taskbar']:taskBar(25000, 'Cooking the Patty')
...
AddEventHandler("npc-burgershot:startfryer", function()
    ...
    if rank > 0 then  
        if exports["npc-inventory"]:hasEnoughOfItem("potato", 1) then
            ...
            TaskPlayAnim(GetPlayerPed(-1), dict, "pushcar_offcliff_f", 3.0, -8, -1, 63, 0, 0, 0, 0 )
            SetEntityHeading(GetPlayerPed(-1), 124.72439575195)
            local finished = exports['npc-taskbar']:taskBar(20000, 'Dropping Fries')
            if (finished == 100) then
...
AddEventHandler("npc-burgershot:makeshake", function()
    ...
    if rank > 0 then
        if exports["npc-inventory"]:hasEnoughOfItem("milkshakeformula", 1) then
            ...
            SetEntityHeading(GetPlayerPed(-1), 124.72439575195)
            local finished = exports['npc-taskbar']:taskBar(10000, 'Making Milk Shake')
            if (finished == 100) then
...
AddEventHandler("npc-burgershot:getcola", function()
    ...
    if rank > 0 then
        if exports["npc-inventory"]:hasEnoughOfItem("hfcs", 1) then   
            ...
            TaskPlayAnim(GetPlayerPed(-1), dict, "handshake_guy_a", 3.0, -8, -1, 63, 0, 0, 0, 0 )
            SetEntityHeading(GetPlayerPed(-1), 124.72439575195)
            local finished = exports['npc-taskbar']:taskBar(5000, 'Getting Coke')
            if (finished == 100) then
...
AddEventHandler("burgershot:register", function(registerID)
    ...
    if myJob == "burger_shot" then
        local order = exports["npc-applications"]:KeyboardInput({
