...
AddEventHandler("sellfish", function()
    if not DoingTask2 then
        DoingTask2 = true
        if exports["npc-inventory"]:hasEnoughOfItem("sushiroll",5,false) then 
            local finished = exports["npc-taskbar"]:taskBar(4000,"Selling Fish")
            if finished == 100 then
                local sushi = math.random(30, 50)
                DoingTask2 = false
                TriggerEvent("inventory:removeItem", "sushiroll", 5)
                TriggerServerEvent('npc-fish:sellSushi', sushi)
            else
                DoingTask2 = false
            end
        end
...
CanFish = true
RegisterNetEvent("npc-fish:tryToFish")
AddEventHandler("npc-fish:tryToFish", function()
    if CanFish then
        if poleTimer == 0 then 
            TryToFish()
        end
...
RegisterNetEvent("npc-fish:allowed")
AddEventHandler("npc-fish:allowed", function(allowed)
    CanFish = allowed
end)
