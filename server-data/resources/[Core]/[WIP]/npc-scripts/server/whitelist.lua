...
function WhiteList.LoadEmsPd(callback)
    ...
    exports.ghmattimysql:execute(q, function(result)
        ...
        for _, data in ipairs(result) do 
            ...
            local job = data.job
            if job == "therapist" then WhiteList.Emergency["therapist"][steamid] = true end
            if job == "doctor" then WhiteList.Emergency["doctor"][steamid] = true end
            if job == "judge" then WhiteList.Emergency["judge"][steamid] = true end
...
function WhiteList.EmergencyPriority(src)
    ...
    if isDoctor then isEmergency = true end
    if isJudge then isEmergency = true end
    if isTher then isEmergency = true end
    local power
