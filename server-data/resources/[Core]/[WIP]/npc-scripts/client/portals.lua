...
RegisterNetEvent('npc-portalsrecieve:coords')
AddEventHandler('npc-portalsrecieve:coords', function(coords1, coords2, coords3, coords4, coords5, coords6, coords7)
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		if distance <= 0.5 then
			if exports["npc-inventory"]:hasEnoughOfItem("key", 1) then 
...
AddEventHandler("gangteleport", function()
        ...
        for k,v in pairs(weapons) do
            if exports["npc-inventory"]:getQuantity(v) ~= 0 then
            weapon = true
            end
        end
