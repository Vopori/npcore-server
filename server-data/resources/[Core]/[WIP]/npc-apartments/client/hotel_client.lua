...
RegisterNetEvent('npc-motels:clMotelUpdate')
AddEventHandler('npc-motels:clMotelUpdate', function(rType)
	roomType = rType
end)
		...
		if insideApartment or comparedst < 100 then
			...
			if (entry4rd < 5 and myRoomType == 3) or (entry3rd < 5 and myRoomType == 3) or (entry1st < 35.0 and myRoomType == 1) or (entry5rd < 65.0 and myRoomType == 1) or (entry2nd < 5 and myRoomType == 2) or (entry6th < 81.0 and myRoomType == 1) then
				...
				if myRoomType == 2 then
					if nearType2 then
						if myRoomLock then
							TriggerEvent('npc-ui:ShowUI', 'show', ("["..Controlkey["housingMain"][2].."] %s"):format("Enter | ["..Controlkey["housingSecondary"][2].."] UNLOCK"))
						else
							TriggerEvent('npc-ui:ShowUI', 'show', ("["..Controlkey["housingMain"][2].."] %s"):format("Enter | ["..Controlkey["housingSecondary"][2].."] LOCK"))
						end
					end
				end
...
Citizen.CreateThread(function()
    exports["npc-polyzone"]:AddBoxZone("apartment_enter", vector3(-1447.72, -537.35, 34.74), 2.0, 1, {
