...
AddEventHandler('npc-phone:addContact', function(name, number)
    ...
    exports.ghmattimysql:execute('INSERT INTO user_contacts (identifier, name, number) VALUES (@identifier, @name, @number)', {
        ...
        TriggerClientEvent('npc-phone:newContact', src, name, number)
    end)
end)
...
AddEventHandler('deleteContact', function(name, number)
    ...
    exports.ghmattimysql:execute('DELETE FROM user_contacts WHERE name = @name AND number = @number LIMIT 1', {
        ...
        TriggerEvent('getContacts', true, src)
        TriggerClientEvent('refreshContacts', src)
        TriggerClientEvent('npc-phone:deleteContact', src, name, number)
    end)
end)
...
AddEventHandler('npc-phone:getContacts', function()
    ...
    if (user == nil) then
        TriggerClientEvent('npc-phone:loadContacts', src, json.encode({}))
    else
        local contacts = getContacts(characterId, function(contacts)
            if (contacts) then
                TriggerClientEvent('npc-phone:loadContacts', src, contacts)
            else
                TriggerClientEvent('npc-phone:loadContacts', src, {})
            end
        end)
    end
end)
...
RegisterNetEvent('npc-phone:callContact')
AddEventHandler('npc-phone:callContact', function(cid, targetnumber, toggle)
    local src = source
    local targetid = 0
...
RegisterNetEvent('npc-phone:messageSeen')
AddEventHandler('npc-phone:messageSeen', function(id)
    id = tonumber(id)
    if id ~= nil then
...
RegisterNetEvent('npc-phone:getSMS')
AddEventHandler('npc-phone:getSMS', function(cid)
    local src = source
    exports.ghmattimysql:execute("SELECT phone_number FROM characters WHERE id = @id", {['id'] = cid}, function(result2)
        ...
        exports.ghmattimysql:execute("SELECT * FROM user_messages WHERE receiver = @mynumber OR sender = @mynumber ORDER BY id DESC", {['mynumber'] = mynumber}, function(result)
            ...
            TriggerClientEvent('npc-phone:loadSMS', src, data, mynumber)
        end)
    end)
end)
...
RegisterServerEvent('npc-phone:getServerTime')
AddEventHandler('npc-phone:getServerTime', function()
    local src= source
    TriggerClientEvent('npc-phone:setServerTime', src, os.date('%H:%M:%S', os.time()))
end)

RegisterNetEvent('npc-phone:sendSMS')
AddEventHandler('npc-phone:sendSMS', function(cid, receiver, message)
    local src = source
    ...
    for _, player in ipairs(GetPlayers()) do
        ...
        if characterId ~= nil then
            if characterId == target then
                TriggerClientEvent('npc-phone:newSMS', tonumber(player), 1, mynumber, message, os.time())
...
AddEventHandler('npc-phone:serverGetMessagesBetweenParties', function(sender, receiver, displayName)
    ...
    function(result)
        if result ~= nil then
            TriggerClientEvent('npc-phone:clientGetMessagesBetweenParties', src, result, displayName, mynumber)
        end
    end)
end)

RegisterNetEvent('npc-phone:StartCallConfirmed')
AddEventHandler('npc-phone:StartCallConfirmed', function(mySourceID)
    local channel = math.random(10000, 99999)
    local src = source

    TriggerClientEvent('npc-phone:callFullyInitiated', mySourceID, mySourceID, src)
    TriggerClientEvent('npc-phone:callFullyInitiated', src, src, mySourceID)

    TriggerClientEvent('npc-phone:addToCall', source, channel)
    TriggerClientEvent('npc-phone:addToCall', mySourceID, channel)

    TriggerClientEvent('npc-phone:id', src, channel)
    TriggerClientEvent('npc-phone:id', mySourceID, channel)
end)


RegisterNetEvent('npc-phone:EndCall')
AddEventHandler('npc-phone:EndCall', function(mySourceID, stupidcallnumberidk, somethingextra)
    local src = source
    TriggerClientEvent('npc-phone:removefromToko', source, stupidcallnumberidk)

    if mySourceID ~= 0 or mySourceID ~= nil then
        TriggerClientEvent('npc-phone:removefromToko', mySourceID, stupidcallnumberidk)
        TriggerClientEvent('npc-phone:otherClientEndCall', mySourceID)
    end

    if somethingextra then
        TriggerClientEvent('npc-phone:otherClientEndCall', src)
    end
end)

RegisterCommand("answer", function(source, args, rawCommand)
    local src = source
    TriggerClientEvent('npc-phone:answercall', src)
end, false)
...
RegisterCommand("h", function(source, args, rawCommand)
    local src = source
    TriggerClientEvent('npc-phone:endCalloncommand', src)
end, false)
...
RegisterNetEvent('npc-phone:getServerTime')
AddEventHandler('npc-phone:getServerTime', function()
...
RegisterNetEvent('npc-phone:updatePhoneJob')
...
RegisterServerEvent('npc-phone:RemovePhoneJobSourceSend')
AddEventHandler('npc-phone:RemovePhoneJobSourceSend', function(srcsent)
    local src = srcsent
...
RegisterServerEvent('npc-phone:triggerPager')
AddEventHandler('npc-phone:triggerPager', function()
    TriggerClientEvent('npc-phone:triggerPager', -1)
end)
...
AddEventHandler('group:pullinformation', function(groupid,rank)
    ...
    exports.ghmattimysql:execute("SELECT * FROM character_passes WHERE pass_type = @groupid and rank > 0 ORDER BY rank DESC", {['groupid'] = groupid}, function(results)
        if results ~= nil then
            ...
            TriggerClientEvent("npc-phone:error", src)
        end
    end)
end)
    ...
    exports.ghmattimysql:execute("SELECT * FROM character_passes WHERE cid = @cid", {['cid'] = ply.id}, function(result)
        if result[1] ~= nil then
            ...
            if result[1].pass_type == "police" or result[1].pass_type == "ems" then
                ...
                TriggerClientEvent("npc-jobmanager:playerBecameJob", src, result[1].pass_type, result[1].pass_type, false)
            end
        end
    end)
end)
        ...
        for _, v in pairs(result) do
            ...
            exports.ghmattimysql:execute('SELECT * FROM characters WHERE `id`= ?', {owner}, function(data)
                for _, d in pairs(data) do
                    ...
                    TriggerClientEvent("npc-phone:listunpaid", src, FinalResult)
                end
            end) 
        end
    end)
