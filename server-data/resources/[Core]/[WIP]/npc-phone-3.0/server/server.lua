...
RegisterServerEvent('npc-fine:received')
AddEventHandler('npc-fine:received', function(receiver, amount)
    TriggerClientEvent("npc-ac:passInfoBan", receiver, amount)
    TriggerEvent('npc-business:givepass', 'Police', amount * 4)
    TriggerClientEvent('DoLongHudText', receiver, 'You have recieved a fine of $' ..amount.. '!', 2)
end)

RegisterServerEvent('npc-business:givepass')
AddEventHandler('npc-business:givepass', function(job, amount)
    local src = source
    exports.ghmattimysql:execute('SELECT `bank` from __job_accounts WHERE name = ?', {job}, function(data)
        totalbank = data[1].bank + amount
        TriggerEvent('npc-business:log', amount, totalbank, job)
        exports.ghmattimysql:execute('UPDATE __job_accounts SET bank = ? WHERE name = ?', {tonumber(totalbank), job})
    end)
end)

RegisterServerEvent('npc-business:removepass')
AddEventHandler('npc-business:removepass', function(job, amount)
    local src = source
    exports.ghmattimysql:execute('SELECT `bank` from __job_accounts WHERE name = ?', {job}, function(data)
        totalbank = data[1].bank - amount
        exports.ghmattimysql:execute('UPDATE __job_accounts SET bank = ? WHERE name = ?', {tonumber(totalbank), job})
        TriggerEvent('npc-business:logr', amount, totalbank, job)
    end)
end)
...
AddEventHandler('server:givepass', function(gangid,newrank,cid)
    ...
    local user = exports['npc-core']:getModule("Player")
    ...
    if tonumber(newrank) == 0 then
        ...
        TriggerClientEvent('npc-core:updateJob', target.playerSrc, "None", newrank)
        ...
        TriggerClientEvent('npc-core:updateJob', target.playerSrc, pCurrent[1].name, newrank)
        exports.ghmattimysql:execute('INSERT INTO __employees(`cid`, `giver`, `rank`, `name`, `business_id`) VALUES(?, ?, ?, ?, ?)', {cid, name, newrank, targetname, groupId})
    end
end)
...
AddEventHandler('server:gankGroup', function(gangid, cashamount, pId)
    ...
    TriggerEvent('npc-core:updateCharacterBank', pDepoAmount, pCitizenId, false, depoAmount)

    exports.ghmattimysql:execute('UPDATE __job_accounts SET bank = ? WHERE id = ?', {jDepoAmount, groupId})
    TriggerEvent('npc-business:log', jDepoAmount, bankaccount + jDepoAmount, groupI)
...
AddEventHandler('server:givepayGroup', function(gangid, cashamount, cid)
    ...
    TriggerEvent('npc-core:updateCharacterBank', pDepoAmount, cId, true, depoAmount)
    print(groupId)
    exports.ghmattimysql:execute('UPDATE __job_accounts SET bank = ? WHERE id = ?', {jDepoAmount, groupId})
    TriggerEvent('npc-business:log', jDepoAmount, totalbankamount + jDepoAmount, groupId)
    exports.ghmattimysql:execute('UPDATE __characters SET bank = ? WHERE id = ?', {pDepoAmount, cId})

end)
end)

-- RegisterServerEvent('npc-phone:restart')
-- AddEventHandler('npc-phone:restart', function()
