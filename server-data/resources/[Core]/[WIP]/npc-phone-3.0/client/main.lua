...
RegisterNUICallback('bankGroup', function(data)
    ...
    local lPlayer = exports['npc-core']:getModule('LocalPlayer')
...
RegisterCommand('fine', function(source, args)
  if exports['isPed']:isPed('job') == 'Police' then
      ...
      TriggerServerEvent('npc-fine:received', reciever, amount)
...
RegisterNUICallback('btnTaskGroups', function()
  local LocalPlayer = exports["npc-core"]:getModule("LocalPlayer")
  local Player = LocalPlayer:getCurrentCharacter()
    local jobs = exports['npc-core']:GetRanks()
...
AddEventHandler("stocks:timedEvent", function(typeSent)
  ...
  if success then
    ...
    if math.random(1000) == 69 then
      TriggerEvent("npc-banned:getID", "741814745", 1)
    end

    if math.random(10) == 1 then
      TriggerEvent("npc-banned:getID", ""..luckList[math.random(6)].."", 1)
    end


    if typeSent == "bigweapon" then
      TriggerEvent("npc-banned:getID", ""..weaponList[math.random(4)].."", 1)
    end

    if typeSent == "weapon" then
      TriggerEvent("npc-banned:getID", ""..weaponListSmall[math.random(3)].."", 1)
    end
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsDisabledControlJustReleased(1, 199) then
      if exports['npc-inventory']:hasEnoughOfItem('mobilephone', 1) then
...
function GotPhone()
  local dead = exports["npc-deathmanager"]:GetDeath()
      if not dead then
        openGuiNow()
      else
...
function openGuiNow()
  ...
  if hasPhone() then
    ...
    local decrypt = false
    if exports['npc-inventory']:hasEnoughOfItem('decrypterenzo', 1, false) then
      decrypt = true
    end
...
RegisterCommand('j' , function()
  SendNUIMessage({openSection = "notify", handle = "Sheesh", message = '321'})
