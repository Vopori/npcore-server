...
function NPC.DB.PhoneNumberExists(self, src, phone_number, callback)
    ...
    if not user then callback(false, true) print("Error!") return end
    ...
    exports.ghmattimysql:execute(q, v, function(results)
        if not results then callback(false, true) print('Exorts Error!') return end
...
function NPC.DB.FetchCharacterData(self, user, callback)
    ...
    exports.ghmattimysql:execute(q,v, function(results)
        if not user then callback(false, true) print('We have an error!') return end
        results = results[1] and results or {}
        callback(results)
    end)
end
