...
local function AddMethod(player)
    ...
    function player.networkVar(self, var, data)
        ...
        TriggerClientEvent("npc-core:networkVar", GetUser(self):getVar("source"), var, data)
    end
    ...
    function player.addMoney(self, amt)
        ...
        NPC.DB:UpdateCharacterMoney(GetUser(self), characterId, cash, function(updatedMoney, err) 
            if updatedMoney then
                ...
                exports["npc-core"]:AddLog("Cash Added", GetUser(self), "Money added to user, amount: " .. tostring(amt))
            end
        end)
    end
        ...
        GetUser(self).character.cash = GetUser(self).character.cash - amt
            ...
            NPC.DB:UpdateCharacterMoney(GetUser(self), characterId, cash, function(updatedMoney, err) 
                if updatedMoney then
                    ...
                    TriggerClientEvent("banking:updateCash", GetUser(self).source, GetUser(self):getCash(), amt)
                    exports["npc-core"]:AddLog("Cash Removed", GetUser(self), "Money removed from user, amount: " .. tostring(amt))
                end
            end)
    end
    ...
    function player.removeBank(self, amt)
        ...
        NPC.DB:UpdateCharacterBank(GetUser(self), characterId, bank, function(updatedMoney, err) 
            if updatedMoney then
                ...
                exports["npc-core"]:AddLog("Bank Removed", GetUser(self), "Bank removed from user, amount: " .. tostring(amt))
            end
        end)
    end
    ...
    function player.addBank(self, amt)
        ...
        NPC.DB:UpdateCharacterBank(GetUser(self), characterId, bank, function(updatedMoney, err) 
            if updatedMoney then
                ...
                TriggerClientEvent("banking:updateBalance", GetUser(self).source, GetUser(self):getBalance(), amt)
                exports["npc-core"]:AddLog("Bank Added", GetUser(self), "Bank added to user, amount: " .. tostring(amt))
            end
        end)
    end
...
local pos = {}
RegisterServerEvent('npc-core:updatecoords')
AddEventHandler('npc-core:updateCoords', function(x,y,z)
    local src = source
    pos[src] = {x,y,z}
end)
...
AddEventHandler("playerDropped", function(reason)
    ...
    TriggerEvent('npc-core:playerDropped', src, user)
end)
